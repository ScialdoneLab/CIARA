...
```
It is also possible to run a [command line implementation](https://github.com/ScialdoneLab/CIARA/blob/master/vignettes/Entropy_server_december_2021.R) of the functions **Get_background_full**, **CIARA_gene** and **CIARA**  with the following:
...
**interactive_plot**. For more exhaustive information about the functions offered by CIARA for visualization  see **Tutorials section** below and the help page of the single functions. (*?function_name*).
...
We can visualize in an interactive way which are the genes highly localized in a particular region of the umap plot with the function **interactive_plot**.
...
3. **Merging of step 1 and 2**: Any cluster identified at step 2 of size  smaller than **n_cells_high** constitutes an independent cluster.
