...
ciara_genes_old=row.names(result_old)[result_old[,1]<1]
#geni_top=row.names(result_old)[result_old[,2]<0.0001&result_old[,1]==0]
geni_top_top=row.names(result_old)[order(as.numeric(result_old[,1]))]
```

```{r }
background_old=row.names(result_old)
#background=c(names(ciara_genes),names(ciara_genes_excluded))
```
...
ciara_genes_old=data_all_human[ciara_genes_old,2]
...
```{r }
plot_genes_sum(umap_cordinate_heart_small,norm_data_heart,(ciara_genes_old),"Sum from top CIARA genes")
...
norm_counts_small = apply(norm_data_heart[(ciara_genes_old), ], 
        1, function(x) {
            y = x/sum(x)
            return(y)
        })
...
genes_name_text=selection_localized_genes(norm_data_heart,ciara_genes_old,min_number_cells=4,max_number_genes=4)
...
heart_seurat_ciara=cluster_analysis_integrate_rare(raw_data_heart,"Elmir data",0.1,5,30,ciara_genes_old)
