    ...
    max_size = 5, text_size = 7) 
{
    ...
    p + ggplot2::geom_point(ggplot2::aes(size = values, colour = as.numeric(all_markers_values))) + 
        ...
        size = "Value (fraction cells above 0.001 log2 norm counts)") + 
...
markers_first_elmir=markers_cluster_seurat(mayra_seurat_24_update,Final_Clusters,names(mayra_seurat_24_update$RNA_snn_res.0.1),5)
```
...
```{r }
norm_counts_small = apply(norm_counts_ane_24_update, 
        1, function(x) {
            y = x/sum(x)
            return(y)
        })
...
plot_balloon_marker(norm_counts_ane_24_update,Final_Clusters,marker_all_day_24_first,5,max_size=5,text_size=10)
...
## End cluster and baloon plot analysis
...
```{r}
DefaultAssay(mayra_seurat_24_update) <- "RNA"
cell_select=colnames(norm_filter)
index_select=colnames(norm_counts_ane_24_update)%in%cell_select
cell_new_sort=colnames(norm_counts_ane_24_update)[index_select]
meta_clu=data.frame(cell_select,finalCluster)
row.names(meta_clu)=cell_select
meta_clu=meta_clu[cell_new_sort,2]
markers_first_elmir=markers_cluster_seurat(mayra_seurat_24_update[,index_select],meta_clu,names(mayra_seurat_24_update$RNA_snn_res.0.1)[index_select],5)
```
...
plot_balloon_marker_norm(norm_counts_small[,index_select],meta_clu,marker_all_day_24_first,5,max_size=5,text_size=10)

plot_balloon_marker(norm_counts_ane_24_update[,index_select],meta_clu,marker_all_day_24_first,4,max_size=5,text_size=10)
...
```{r }
plot_umap(cordinate_umap_24_update[index_select,],meta_clu)
```

##End cluster analysis and baloon plot
...
## Plot umap and baloon plot of cluster from Gini3

```{r}
DefaultAssay(mayra_seurat_24_update) <- "RNA"
markers_first_elmir=markers_cluster_seurat(mayra_seurat_24_update,final_gini,names(mayra_seurat_24_update$RNA_snn_res.0.1),5)
```
...
plot_balloon_marker(norm_counts_ane_24_update,final_gini,marker_all_day_24_first,5,max_size=5,text_size=10)

#plot_balloon_marker_norm(norm_counts_small[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(to#Match,collapse="|"),original_all)],markers_first_elmir_3_final,5,max_size=5,text_size=10)
```
## Stp cluster anlysis and baloon plot
...
sam_genes=read.csv("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/published_data/sam_output/sam_rank_genes_mouse.csv")
sam_genes=sam_genes$X0

sam_cluster=read.csv("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/published_data/sam_output/sam_clustering_mouse.csv")
...
#load(file="knn_matrix_ane_24_update.Rda")
load(file="cordinate_umap_24_update.Rda")
plot_umap(cordinate_umap_24_update,sam_cluster)

```
## Cluster analysis and baloon plots with SALMON
```{r}
DefaultAssay(mayra_seurat_24_update) <- "RNA"
markers_first_elmir=markers_cluster_seurat(mayra_seurat_24_update,sam_cluster,names(mayra_seurat_24_update$RNA_snn_res.0.1),5)
```
...
plot_balloon_marker_norm(norm_counts_small,sam_cluster,marker_all_day_24_first,5,max_size=5,text_size=10)

plot_balloon_marker(norm_counts_ane_24_update,sam_cluster,marker_all_day_24_first,5,max_size=5,text_size=10)
...
## Cluster and baloon plot
```{r}
DefaultAssay(mayra_seurat_24_update) <- "RNA"
markers_first_elmir=markers_cluster_seurat(mayra_seurat_24_update,sc@cpart,names(mayra_seurat_24_update$RNA_snn_res.0.1),5)
```
...
plot_balloon_marker_norm(norm_counts_small,sc@cpart,marker_all_day_24_first,5,max_size=5,text_size=10)

plot_balloon_marker(norm_counts_ane_24_update,sc@cpart,marker_all_day_24_first,5,max_size=5,text_size=10)
