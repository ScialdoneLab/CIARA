...
```{r }
plot_interactive_here=function (coordinate_umap, color, text, min_x = NULL, max_x = NULL, 
    min_y = NULL, max_y = NULL) 
{
    ...
    fig <- plotly::plot_ly(data = coordinate_umap, x = ~UMAP_1, 
        ...
        marker = list(size = 5, width = 2), text = ~text, hoverinfo = "text",colors='Blues')
    if (!is.null(min_x)) {
...
```{r }
#plot_interactive
gene_sum=rep(0,length(genes_name_text))
for (i in 1:length(genes_name_text)){
top_genes=strsplit(genes_name_text[i],"-")[[1]]
if (all(top_genes%in%colnames(norm_counts_small))){
  gene_sum[i] = sum(norm_counts_small[i,top_genes])}
else {gene_sum[i]=0}
...
```{r }
norm_counts_small <- apply(norm_counts_ane_0[ciara_genes, ], 1, function(x) {
  y <- x/sum(x)
  return(y)
  })
gene_sum <- apply(norm_counts_small, 1, sum)
...
genes_name_text <- selection_localized_genes(norm_counts_ane_0, ciara_genes, min_number_cells = 4, max_number_genes = 4)
colnames(cordinate_umap_0) <- c("UMAP_1","UMAP_2")
plot_interactive_here(cordinate_umap_0, gene_sum, genes_name_text, min_x=NULL, max_x=NULL, min_y=NULL, max_y=NULL)
```
