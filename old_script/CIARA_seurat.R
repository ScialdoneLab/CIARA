...
#' @importFrom Seurat GetAssayData Graphs FeaturePlot
...
CIARA_seurat <- function(object, background, cores_number = 1, p_value = 0.001, odds_ratio = 2, local_region = 1, approximation = FALSE) {
  ...
  if (! ("RNA_nn" %in% Graphs(object))){
    stop ("Knn matrix not present in seurat object. Run first the function FindNeighbors")
  }
  knn_matrix <- as.matrix(object[["RNA_nn"]])
  if (! ("CIARA_background" %in% colnames(human_data_seurat[["RNA"]][[]]))) {
    stop("Run first the function get_background_full_seurat before CIARA")
  }
  background <- row.names(human_data_seurat[["RNA"]][['CIARA_background']])[human_data_seurat[["RNA"]][['CIARA_background']][,1]]
