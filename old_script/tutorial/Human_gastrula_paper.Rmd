...
```{r }
CIARA_gene_here=function (norm_matrix, knn_matrix, gene_expression, p_value = 0.001, 
    odds_ratio = 2, local_region = 1, approximation) 
{
    ...
    if (approximation == FALSE) {
        ...
        sub_feature <- colnames(norm_matrix)[which(binary_expression == 
            1)]
    ...
    perform_fisher <- function(j) {
        nn_cell <- colnames(norm_matrix)[which(knn_matrix_big[sub_feature[j], 
            ] > 0)]
        nn_gene_expression <- binary_expression[colnames(norm_matrix) %in% 
            nn_cell]
        if (sum(nn_gene_expression) != 0) {
            input_fisher <- matrix(c(sum(nn_gene_expression == 
                1), sum(binary_expression == 1) - sum(nn_gene_expression == 
                1), sum(nn_gene_expression == 0), sum(binary_expression == 
                0) - sum(nn_gene_expression == 0)), nrow = 2, 
                ncol = 2, byrow = T)
            test_output <- fisher.test(input_fisher, alternative = "g")
            if (test_output$estimate>1){
    ...
    p_value_gene <- unlist(lapply(seq_len(length(sub_feature)), 
        perform_fisher))
    ...
    if (all(p_value_final == 1)) {
        warning(paste0("There are not genes enriched in ", local_region, 
            " or more local regions"))
    }
    return(list(p_value_final))
}
```

```{r }
CIARA_here=function (norm_matrix, knn_matrix, background, cores_number = 1, 
    p_value = 0.001, odds_ratio = 2, local_region = 1, approximation) 
{
    if (!(is.logical(approximation))) {
        stop("approximation must be a logical value. Try with approximation = TRUE or approximation = FALSE")
    }
    run_loop_genes = function(i) {
        ...
        return(CIARA_gene_here(norm_matrix, knn_matrix, gene_expression, 
            p_value, odds_ratio, local_region, approximation))
    }
...
background_here <- get_background_full(norm_elmir_5_30, threshold = 1, n_cells_low = 3, n_cells_high = 20)
result_here <- CIARA_here(norm_elmir_5_30, knn_matrix_elmir_5_30, background_here[1:10], cores_number = 1, p_value = 0.001, odds_ratio = 2, local_region = 1, approximation = TRUE)

result_old <- CIARA(norm_elmir_5_30, knn_matrix_elmir_5_30, background_here[1:10], cores_number = 1, p_value = 0.001, odds_ratio = 2, local_region = 1, approximation = TRUE)

gene_expression <- as.vector(norm_elmir_5_30[background_here[1],])

result_gene_here=CIARA_gene_here(norm_elmir_5_30, knn_matrix_elmir_5_30, gene_expression, p_value = 0.001, odds_ratio = 2, local_region = 1, approximation = TRUE)
...
#gradient1 = colorpanel( sum( breaks[-1]<=2 ), "#FFFFFF",ramp.list[index_color])
...
##Start here for baloon plots for markers YSE1 and MEP1
...
add_genes_endo=c("SERPIND1","SERPINC1", "SERPINF2", "TF")
names(add_genes_endo)=rep("YSE1",length(add_genes_endo))

markers_endo_plot=c(markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="DE1"][1:9],markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="DE2"][1:9],markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="YSE"][1:9],c(markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="YSE1"][1:5],add_genes_endo),markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="Hypoblast"][1:9])

pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figure_4/extended_marker_4c.pdf",width = 8, height = 6)
plot_balloon_marker_norm(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch,collapse="|"),original_all)],markers_endo_plot,9,max_size=2,text_size=8)
dev.off()
...
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/YSE1_big.pdf",width = 8, height = 6)
#plot_balloon_marker_here(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch#,collapse="|"),original_all)],top_endo,10,max_size=8,text_size=7)

plot_balloon_marker_norm(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch,collapse="|"),original_all)],c(top_endo[1],top_endo[3:11]),10,max_size=12,text_size=25)
dev.off()

##Other version
add_genes_endo=c("SERPIND1","SERPINC1", "SERPINF2", "TF")
names(add_genes_endo)=rep("YSE1",length(add_genes_endo))
markers_endo_plot=c(markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="YSE1"][6:12],add_genes_endo)

plot_balloon_marker_norm(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch,collapse="|"),original_all)],markers_endo_plot,10,max_size=10,text_size=25)

markers_endo_plot=c(markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="YSE1"][1:5])
plot_balloon_marker_norm(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch,collapse="|"),original_all)],markers_endo_plot,5,max_size=10,text_size=16)
...
plot_balloon_marker_norm_receptor=function (norm_counts, cluster, marker_complete, max_number, 
    max_size = 5, text_size = 7) 
{
    livelli <- c("DE1","DE2","YSE","YSE1","Hypoblast","Erythroblasts")
...
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/receptor.pdf",width = 8, height = 6)
plot_balloon_marker_norm_receptor(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],new_clu_plot,receptor_plot,3,max_size=12,text_size=25)
dev.off()

#plot_balloon_marker_here(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch#,collapse="|"),original_all)],receptor_plot,3,max_size=8,text_size=10)
...
markers_endo_plot=c(markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="MEP1"][1:5])
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/Figure_4/marker_blood.pdf",width = 8, height = 6)
plot_balloon_marker_norm_blood(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch,collapse="|"),original_all)],markers_endo_plot,5,max_size=12,text_size=25)
dev.off()
```
...
load(file="top_hemo.Rda")
names(top_hemo)=rep("MEP1",length(top_hemo))
markers_endo_plot_big=c(top_hemo[2:4],add_genes_hemo,add_2)

#pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figure_4/extended_marker_4h.pdf",width = 8, height = 6)
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/hemo_big_mouse.pdf",width = 8, height = 6)
plot_balloon_marker_norm_blood(norm_elmir_5_30[,grep(paste(toMatch,collapse="|"),original_all)],original_sub[grep(paste(toMatch,collapse="|"),original_all)],markers_endo_plot_big,10,max_size=12,text_size=25)
dev.off()
```
...
select_all=which(as.vector(umap_elmir$cluster_id!="Hemogenic Endothelial Progenitors")&as.vector(umap_elmir$cluster_id!="Endoderm")&as.vector(umap_elmir$cluster_id!="Erythroblasts"))
select_endo=which(as.vector(umap_elmir$cluster_id=="Endoderm"))
select_blood=which(as.vector(umap_elmir$cluster_id=="Hemogenic Endothelial Progenitors"))
levels(factor(original_sub[select_blood]))
#select_all=c(select_blood,select_endo)
original_umap=original_sub
original_umap[select_all]="Other cluster"

p=plot_umap(cordinate_umap,original_umap)


pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figure_4/figure_4_b_new.pdf",width = 8,height = 6)
p[[1]]+theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.text.y=element_blank(),
        axis.ticks.y=element_blank())+theme(text = element_text(size=18))+scale_colour_manual(breaks = c("EMP","Ery","HE","MEP" ,"MEP1","MP","DE1","DE2","Hypo","YSE","YSE1","Other cluster"),values=c("#F8766D","#B79F00","#00BA38","#00BFC4","#619CFF","#F564E3","#42152C","#AA7EAF","#F2CBBC","#B48371","#FEFD54","grey"))
dev.off()
