...
```{r }
library(gplots)
localized_genes_mouse=detect_localized_genes(knn_matrix_gini_5_30,norm_gini,geni_top_top,10)  
...
p=plot_interactive_here(cordinate_gini,norm_gini,rank_intersect,genes_name_text,hm.colors,min_x=NULL,max_x=NULL,min_y=NULL,max_y=NULL)

htmlwidgets::saveWidget(p, file = "gini_2clc.html")

p=plot_genes_sum_here(cordinate_gini,norm_gini,rank_intersect,"Top genes CIARA",hm.colors)

pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/interactive_plot/gini_2clc.pdf",width = 8,height = 6)
...
##Skip below (just a try for figure 1)
```{r }

gini_normal=cluster_analysis_integrate_rare(test_data_1,"24h",0.1,3,30)
...
```{r }
library(clustree)
#find_resolution(gini_normal,seq(0.0001,0.0002,0.0001))
```

```{r }

norm_gini_normal=as.matrix(GetAssayData(gini_normal, slot = "data",assay="RNA"))

#norm_gini_normal=norm_gini_normal[,1:2484]

knn_matrix__gini_normal=as.matrix(gini_normal@graphs$RNA_nn)

cordinate_gini_normal=as.data.frame(Embeddings(gini_normal, reduction = "umap")[, 1:2])

#cordinate_gini_normal=cordinate_gini_normal[1:2484,]
#gini_normal=FindClusters(gini_normal,resolution = 0.000000000000001)
cluster_normal=as.vector(gini_normal$RNA_snn_res.0.01)
#cluster_normal=cluster_normal[1:2484]

setwd("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/published_data")
save(norm_gini_normal,file="norm_gini_normal.Rda")
save(knn_matrix__gini_normal,file="knn_matrix__gini_normal.Rda")
...
setwd("/nas_storage/gabriele.lubatti/ciara_2clc")
load(file="norm_gini_normal.Rda")
load(file="knn_matrix__gini_normal.Rda")


norm_matrix=norm_gini_normal
knn_matrix=knn_matrix__gini_normal

background <- get_background_full(norm_matrix, threshold = 1, n_cells_low = 3, n_cells_high = 20)
result <- CIARA(norm_matrix, knn_matrix, background, cores_number = 10, p_value = 0.001, odds_ratio = 2, local_region = 1, approximation = FALSE)
result_gini_normal=result
save(result_gini_normal,file="result_gini_normal.Rda")
...
```{r }
library(gplots)
localized_genes_mouse=detect_localized_genes(knn_matrix__gini_normal,norm_gini_normal,geni_top_top,10)  
...
setwd("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/interactive_plot")
p=plot_interactive_here(cordinate_gini_normal,norm_gini_normal,rank_intersect,genes_name_text,hm.colors,min_x=NULL,max_x=NULL,min_y=NULL,max_y=NULL)

htmlwidgets::saveWidget(p, file = "gini_2clc.html")

p=plot_genes_sum_here(cordinate_gini_normal,norm_gini_normal,rank_intersect,"Top genes CIARA",hm.colors)
...
```{r }
plot_umap(cordinate_gini_normal,cluster_normal)

plot_gene(norm_gini_normal,cordinate_gini_normal,"Zscan4d","Zscan4d")
```

### Plot QC feature
```{r }
Plot_qc_cluster(gini_normal,as.vector(as.vector(gini_normal$RNA_snn_res.0.1)),"mt-","Rps","Rpl")
#Plot_qc_cluster_old(es_combined,as.vector(es_combined$integrated_snn_res.0.1),"mouse")
```



```{r }
norm_gini=as.matrix(GetAssayData(giniclus, slot = "data",assay="RNA"))
norm_gini=norm_gini[,1:2484]
...
#gini_entropy_24=cluster_analysis_integrate_rare(test_data_1,"24h",0.1,3,30,geni_top_top[1:10])
...
```{r }
library(clustree)
find_resolution(gini_entropy_24,seq(0.01,0.03,0.01))
```
...
cluster_vedo[cluster_entropy==5]="2CLC"


plot_umap(cordinate_gini,cluster_vedo)

plot_umap(cordinate_gini,cluster_entropy)
```
...
df=data.frame(c(0.87,0.87),method)
colnames(df)=c("MCC","Method")

pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/mcc_score_1.pdf",width = 8,height = 6)
...
colnames(df)=c("MCC","Method")
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/mcc_score_2.pdf",width = 8,height = 6)
ggplot(data=df, aes(x=Method, y=MCC, fill=Method)) +
  geom_bar(stat="identity", width=0.5)+ylim(0,1)+ggtitle("MCC score")+scale_fill_manual(values=c(CIARA:::gg_color_hue(2)))
dev.off()
```
...
colnames(df)=c("MCC","Method")
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/mcc_score_3.pdf",width = 8,height = 6)
ggplot(data=df, aes(x=Method, y=MCC, fill=Method)) +
  geom_bar(stat="identity", width=0.5)+ylim(0,1)+ggtitle("MCC score")+scale_fill_manual(values=c(CIARA:::gg_color_hue(2)))
dev.off()
```
...
colnames(df)=c("MCC","Method")
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/mcc_score_4.pdf",width = 8,height = 6)
ggplot(data=df, aes(x=Method, y=MCC, fill=Method)) +
  geom_bar(stat="identity", width=0.5)+ylim(0,1)+ggtitle("MCC score")+scale_fill_manual(values=c(CIARA:::gg_color_hue(2)))
dev.off()
```
...
```{r }
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/race_id_umap.pdf",width = 8,height = 6)
p=plot_umap(cordinate_race,cluster_race_entropy)
p[[1]]+theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.text.y=element_blank(),
        axis.ticks.y=element_blank())+
...
markers_first_elmir=markers_cluster_seurat(race_entropy,as.vector(race_entropy$RNA_snn_res.0.2),names(race_entropy$RNA_snn_res.0.2),5)
```
...
```{r }
top_1=white_black_markers(as.vector(race_entropy$RNA_snn_res.0.2),6,norm_race,marker_all_day_24_first,0)

top_1=names(top_1)[top_1]

mean_top_1=apply(norm_race[top_1,as.vector(race_entropy$RNA_snn_res.0.2)==1],1,mean)
...
```{r }
top_2=white_black_markers(as.vector(race_entropy$RNA_snn_res.0.2),7,norm_race,marker_all_day_24_first,0)

top_2=names(top_2)[top_2]

mean_top_2=apply(norm_race[top_2,as.vector(race_entropy$RNA_snn_res.0.2)==2],1,mean)
...
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/marker_cluster_6_raceid.pdf",width = 8, height = 6)
plot_balloon_marker_here(norm_race,as.vector(race_entropy$RNA_snn_res.0.2),c(top_1[1:10]),10,max_size=12,text_size=25)
dev.off()
```

```{r }
#Dclk1, Lrmp, Trpm5
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/marker_cluster_7_raceid.pdf",width = 8,height = 6)
plot_balloon_marker_here(norm_race,as.vector(race_entropy$RNA_snn_res.0.2),c(top_2[1:7],top_2[33],top_2[38],top_2[40]),10,max_size=12,text_size=25)
dev.off()
```
...
classe_new<-data.frame(deepSplit0 = cluster_race_entropy[cluster_race_entropy==7|cluster_race_entropy==6],deepSplit1=cluster_race_id[cluster_race_entropy==7|cluster_race_entropy==6])
...
```{r }
library(clustree)
clustree(classe_new,prefix="deepSplit", node_size = 10,node_label = "algorithm")


label_position <- function(labels) {
    if (length(unique(labels)) == 1) {
        labels[labels==1]="RaceID"
        labels[labels==0]="CIARA"
...
label_position <- function(labels) {
  if (all(unique(labels=)==6)|all(unique(labels=)==7)){
    position= cluster
  }
  else{
    position="RaceID"
...
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/clustree_race.pdf",width = 8,height = 6)
clustree(classe_new, prefix="deepSplit", node_size = 10)
dev.off()

clustree(classe_new, prefix="deepSplit", node_size = 10,node_label = "algorithm",node_label_aggr = "algorithm")

clustree(classe_new, prefix="deepSplit", node_size = 10,node_label_aggr = "algorithm")
...
```{r }
plot_gene(norm_race,cordinate_race,"Kctd12","Smtn")
plot_gene(norm_race,cordinate_race,"Atp1a2","Smtn")
plot_umap(cordinate_race,cluster_race_entropy)
```
...
```
```{r }
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/giniclust2_umap.pdf",width = 8, height = 6)
p=plot_umap(cordinate_umap_gini,as.vector(giniclu2_entropy$RNA_snn_res.0.3))
p[[1]]+theme_bw()+theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"),axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),axis.text.y=element_blank(),
        axis.ticks.y=element_blank())+
...
```{r}
DefaultAssay(giniclu2_entropy) <- "RNA"
markers_first_elmir=markers_cluster_seurat(giniclu2_entropy,as.vector(giniclu2_entropy$RNA_snn_res.0.3),names(giniclu2_entropy$RNA_snn_res.0.3),5)
```
...
```{r }
top_1=white_black_markers(as.vector(giniclu2_entropy$RNA_snn_res.0.3),1,norm_gini2,marker_all_day_24_first,0)

top_1=names(top_1)[top_1]

mean_top_1=apply(norm_gini2[top_1,as.vector(giniclu2_entropy$RNA_snn_res.0.3)==1],1,mean)
mean_top_1=sort(mean_top_1,decreasing = T)
...
```{r }
top_2=white_black_markers(as.vector(giniclu2_entropy$RNA_snn_res.0.3),2,norm_gini2,marker_all_day_24_first,0)

top_2=names(top_2)[top_2]

mean_top_2=apply(norm_gini2[top_2,as.vector(giniclu2_entropy$RNA_snn_res.0.3)==2],1,mean)
mean_top_2=sort(mean_top_2,decreasing = T)
...
```{r }
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/marker_cluster_1_giniclust2.pdf",width = 8, height = 6)
plot_balloon_marker_here(norm_gini2,as.vector(giniclu2_entropy$RNA_snn_res.0.3),c(top_1[1:10]),10,max_size=12,text_size=25)
dev.off()
```

```{r }
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/marker_cluster_2_giniclust2.pdf",width = 8,height = 6)
plot_balloon_marker_here(norm_gini2,as.vector(giniclu2_entropy$RNA_snn_res.0.3),c(top_2[1:10]),10,max_size=12,text_size=25)
dev.off()
```
...
colnames(df)=c("MCC","Method")
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/mcc_score_5.pdf",width = 8,height = 6)
ggplot(data=df, aes(x=Method, y=MCC, fill=Method)) +
  geom_bar(stat="identity", width=0.5)+ylim(0,1)+ggtitle("MCC score")+scale_fill_manual(values=c(CIARA:::gg_color_hue(2)))
dev.off()
```
...
colnames(df)=c("MCC","Method")
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/mcc_score_6.pdf",width = 8,height = 6)
ggplot(data=df, aes(x=Method, y=MCC, fill=Method)) +
  geom_bar(stat="identity", width=0.5)+ylim(0,1)+ggtitle("MCC score")+scale_fill_manual(values=c(CIARA:::gg_color_hue(2)))
dev.off()
```
...
##R and pyhton together
```{r}
old_ratio=n_time_old_r
new_ratio=n_time_old_p
summary_plot=data.frame(old_ratio,new_ratio,n_cells)

n_cells_all=rep(n_cells,2)
#n_cells_all=n_cells
n_time_all=c(old_ratio,new_ratio)
#n_time_all=c(old_ratio)
label_all=c(rep("R",length(n_cells)),rep("python",length(n_cells)))
#label_all=c(rep("Tested dataset",length(n_cells)))
all_data=data.frame(n_cells_all,n_time_all,label_all)
...
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/running_time_comparison.pdf",height = 6, width = 8)
ggplot(all_data, aes(x=n_cells_all, y=n_time_all,color=label_all)) +
  geom_point(size=2)+xlab("Number of cells") + ylab("Running time (minutes)") +ggtitle("CIARA running time: R vs python")+labs(col="setting")+geom_line(aes(colour = label_all), size = 1)+ylim(c(0,11))+scale_y_continuous(breaks=seq(0,11,2),limits = c(0,11))+
  scale_x_continuous(breaks=seq(10,4000,500),limits = c(0,4500))+xlim(c(0,4000))+theme(text = element_text(size=14))
dev.off()
...
#pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/sup_figure_6/sup_fig_6_a_ratio.pdf")
pdf("/Users/gabriele.lubatti/Desktop/Phd/entropy_of_mixing_library/github_january_2022/figures_paper/figures_conference/supplementary_figures/running_time.pdf",height = 6, width = 8)
ggplot(all_data, aes(x=n_cells_all, y=n_time_all,color=label_all)) +
  geom_point(size=2)+xlab("Number of cells") + ylab("Ratio") +ggtitle("CIARA running time: R vs python")+labs(col="setting")+geom_line(aes(colour = label_all), size = 1)+ylim(c(0,11))+scale_y_continuous(breaks=seq(0,4,1),limits = c(0,4))+
  scale_x_continuous(breaks=seq(10,4000,500),limits = c(0,4500))+xlim(c(0,4000))+theme(text = element_text(size=14))+theme(legend.position="none")
dev.off()
...
Rscript --vanilla /home/ies/gabriele.lubatti/ciara_marco/Entropy_server_december_2021_easy.R -n norm_counts_ane_0 -k knn_matrix_ane_0 -o ane_0.Rda -r 1 -l 3 -h 20 -t 1 -p 0.001 -c 10 -a FALSE
...
## Running time pyhton (these are the values used for the plot)
```{r}
##Giniclust fig3b
...
##Running Time for CIARA (these are the values used for the plot)
```{r}
