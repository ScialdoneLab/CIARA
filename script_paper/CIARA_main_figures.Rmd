...
###Note: input for CIARA is always knn 5 PCA:30  for all the datasets.
...
```{r }
Function_fisher_0_48=function(a,b,c){
...
```{r}
Test_Fisher_0_48=function(ciara_cluster,markers_cluster,marker_top){
...
show=Function_fisher_0_48(set_1,set_2,set_all)
...
```{r }
plot_balloon_marker_here_0=function (norm_matrix, cluster, marker_complete, max_number, 
    max_size = 5, text_size = 7) 
{
  #cluster=factor(cluster,levels = c("Pluripotent cells","2CLC","Precursor cells"))
    #livelli <- levels((cluster))
  livelli <- c("Pluripotent cells","2CLC","Precursor cells")
    all_markers_plot <- rep(list(0), length(livelli))
    all_markers <- rep(list(0), length(livelli))
    for (i in 1:length(livelli)) {
        genes_4_0 <- marker_complete[(names(marker_complete) == 
    ...
    colnames(balloon_melted) <- c("all_markers_plot", "cluster_label_all", 
        "values")
    p <- ggplot2::ggplot(balloon_melted, ggplot2::aes(x = factor(cluster_label_all,levels=c(c("Pluripotent cells","2CLC","Precursor cells"))), 
        y = all_markers_plot))
    p + ggplot2::geom_point(ggplot2::aes(size = values, colour = as.numeric(all_markers_values))) + 
        ggplot2::theme(panel.background = ggplot2::element_blank(), 
            panel.border = ggplot2::element_rect(colour = "blue", 
...
p=plot_localized_genes_interactive(cordinate_umap_0,norm_counts_ane_0,rank_intersect,genes_name_text,hm.colors,min_x=NULL,max_x=NULL,min_y=NULL,max_y=NULL)
#setwd("/Users/gabriele.lubatti/Downloads")
#htmlwidgets::saveWidget(p, file = "mouse_esc_0_h.html")

pdf("/Users/gabriele.lubatti/Downloads/mouse_esc_0_h.pdf",width = 8, height = 6)
...
names(top_0)=rep("Pluripotent cells",length(top_0))
```
...
names(top_1)=rep("2CLC",length(top_1))
```
...
names(top_2)=rep("Precursor cells",length(top_2))
```

```{r }
new_clu=as.vector(mayra_0_ciara$RNA_snn_res.0.1)
new_clu[new_clu==0]="Pluripotent cells"
new_clu[new_clu==1]="2CLC"
new_clu[new_clu==2]="Precursor cells"
new_clu=factor(new_clu,levels=c("Pluripotent cells","2CLC","Precursor cells"))
```


```{r }
plot_balloon_marker_here_0(norm_counts_ane_0,new_clu,c(top_1[1:8],top_1[18],top_1[30]),10,max_size=12,text_size=20)

```

```{r }
pdf("/Users/gabriele.lubatti/Downloads/2clc.pdf",width = 8, height = 6)
plot_balloon_marker_here_0(norm_counts_ane_0,new_clu,c(top_1[1:8],top_1[18],top_1[30]),10,max_size=10,text_size=20)
dev.off()
```

```{r }
pdf("/Users/gabriele.lubatti/Downloads/pre_cells.pdf", width = 8, height = 6)
plot_balloon_marker_here_0(norm_counts_ane_0,new_clu,c(top_2[1:9],top_2[17]),10,max_size=10,text_size=20)
dev.off()
```
...
labels <- read.table('labels_jurkat_two_species_1580.txt')
labels=labels$V1
```
...
```{r}
DefaultAssay(race_ciara) <- "RNA"
markers_race_ciara_original=markers_cluster_seurat(race_ciara,cluster_race_id,names(race_ciara$RNA_snn_res.0.2),5)
```
```{r}

marker_all_day_24_first_original=markers_race_ciara_original[[3]]
...
Cluster 7 comprises Lysozyme-expressing Paneth cells while Mucus-producing goblet cells constitute clusters 6 and 10.
```{r}
marker_all_day_24_first[marker_all_day_24_first=="Lgr5"]
```
```{r}
plot_gene(norm_race,cordinate_race,"Lgr5","Lgr5")
plot_umap(cordinate_race,cluster_race_id)
```


### Cluster 2 is Goblet cells (cluster 6 in original cluster))
...
```{r}
marker_all_day_24_first_original[marker_all_day_24_first_original=="Clca3"]
```


### Cluster 3 is enterocytes (cluster 4 in original cluster)
```{r}
marker_all_day_24_first[marker_all_day_24_first=="Apoa1"]
```
```{r}
marker_all_day_24_first_original[marker_all_day_24_first_original=="Apoa1"]
```
### Cluster 4 is Paneth cells (cluster 7 in original cluster)
```{r}
marker_all_day_24_first[marker_all_day_24_first=="Defa24"]
```

```{r}
marker_all_day_24_first_original[marker_all_day_24_first_original=="Defa24"]
```

### Cluster 5 is enteroendocrine cells (also clu 5 in original cluster)
...
```
```{r}
marker_all_day_24_first_original[marker_all_day_24_first_original=="Chgb"]
```
...
```
```{r }
classe_new<-data.frame(deepSplit1 = cluster_race_ciara[cluster_race_ciara==2|cluster_race_ciara==3|cluster_race_ciara==4|cluster_race_ciara==5|cluster_race_ciara==7|cluster_race_ciara==6],deepSplit0=cluster_race_id[cluster_race_ciara==3|cluster_race_ciara==4|cluster_race_ciara==5|cluster_race_ciara==7|cluster_race_ciara==6|cluster_race_ciara==2])
classe_new$K1=c(rep("RaceID",92))
classe_new$K2=c(rep("CIARA",92))





pdf("/Users/gabriele.lubatti/Downloads/race_id_clustree_full.pdf", width = 8, height = 6)
clustree(classe_new, prefix="deepSplit", node_size = 10)
dev.off()
...
```
```{r}
pdf("/Users/gabriele.lubatti/Downloads/resolution_pgc.pdf",width = 8, height = 6)
find_resolution(elmir_seurat_ciara, resolution_vector = seq(0.01,1,0.1))
dev.off()
...
setwd(paste0(current_wd,"/CIARA"))
sam_genes=read.csv("sam_rank_genes_human.csv")
...
sam_cluster=read.csv("sam_clustering_human.csv")
...
table(final_cluster_version_sub[vedo[[2]][[1]]])

table(final_cluster_version_sub[vedo[[2]][[2]]])
table(final_cluster_version_sub[vedo[[2]][[3]]])
table(final_cluster_version_sub[vedo[[2]][[4]]])
table(final_cluster_version_sub[vedo[[2]][[5]]])
...
geni_marker=geni_pgc
positive_cells=find_positive_cells(norm_elmir_ciara_5_30,threshold=0,geni_marker)
Final_Clusters=vedo[[2]]

ciara_score=rep(0,length(colnames(norm_elmir_ciara_5_30)))
original_score=rep(0,length(colnames(norm_elmir_ciara_5_30)))
result_method=rep(0,length((Final_Clusters)))
...
method=factor(c("CIARA","CellSIUS","RaceID","GiniClust2","GiniClust3","SAM","FiRE","GapClust"),levels=c(c("CIARA","CellSIUS","RaceID","GiniClust2","GiniClust3","SAM","FiRE","GapClust")))
df=data.frame(c(1,0.17,0.53,0.11,0.44,0.30,-0.002221461,0.7702834),method)
colnames(df)=c("MCC","Method")

pdf("/Users/gabriele.lubatti/Downloads/mcc_gap_clust.pdf",width = 8, height = 6)
ggplot(data=df, aes(x=Method, y=MCC)) +
  geom_bar(stat="identity", width=0.5)+ylim(-0.01,1)+ggtitle("MCC score")+ggplot2::theme(text = ggplot2::element_text(size = 18), 
            axis.text.x = ggplot2::element_text(angle = 45, vjust = 1, 
                hjust = 1), axis.title.x = ggplot2::element_blank())
...
pdf("/Users/gabriele.lubatti/Downloads/index.pdf",width = 8, height = 6)
...
```{r}
diff_marker=marker_all_day_24_first_all[names(marker_all_day_24_first_all)=="2-step_2"]
#setwd("/Users/gabriele.lubatti/Downloads")
#write.csv2(diff_marker,file="Precursor_markers_24h.csv",row.names = F, col.names = F)
```
```{r}
two_marker=marker_all_day_24_first_all[names(marker_all_day_24_first_all)=="1-step_2"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(two_marker,file="2CLC_markers_24h.csv",row.names = F, col.names = F)
```

```{r}
pluri_marker=marker_all_day_24_first_all[names(marker_all_day_24_first_all)=="0"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(pluri_marker,file="Pluripotent_markers_24h.csv",row.names = F, col.names = F)
```
...
pdf("/Users/gabriele.lubatti/Downloads/dendo_0_24_48.pdf",width = 8, height = 6)
hc <- hclust(my.dist)
plot(hc)
dev.off()
...
```{r}
marker_plot_day_0_first=markers_first_elmir[[1]]

marker_plot_plot_day_0_first=markers_first_elmir[[2]]
marker_all_day_0_first=markers_first_elmir[[3]]
marker_all_day_0_first_2=marker_all_day_0_first[names(marker_all_day_0_first)==2]
```


```{r}
diff_marker=marker_all_day_0_first[names(marker_all_day_0_first)=="2"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(diff_marker,file="Precursor_markers_0h.csv",row.names = F, col.names = F)
```
```{r}
two_marker=marker_all_day_0_first[names(marker_all_day_0_first)=="1"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(two_marker,file="2CLC_markers_0h.csv",row.names = F, col.names = F)
```

```{r}
pluri_marker=marker_all_day_0_first[names(marker_all_day_0_first)=="0"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(pluri_marker,file="Pluripotent_markers_0h.csv",row.names = F, col.names = F)
```

```{r}


test=Test_Fisher_0_48(final_cluster_version_sub,marker_all_day_24_first_all,marker_all_day_0_first_2)
test[[2]][test[[1]]<0.05&test[[2]]>2]
```
```{r}

marker_all_day_0_first_1=marker_all_day_0_first[names(marker_all_day_0_first)==1]
test=Test_Fisher_0_48(final_cluster_version_sub,marker_all_day_24_first_all,marker_all_day_0_first_1)
test[[2]][test[[1]]<0.05&test[[2]]>2]
#1.132057e-44 p value
```
```{r}

marker_all_day_0_first_0=marker_all_day_0_first[names(marker_all_day_0_first)==0]
test=Test_Fisher_0_48(final_cluster_version_sub,marker_all_day_24_first_all,marker_all_day_0_first_0)
test[[2]][test[[1]]<0.05&test[[2]]>2]
#0.0001163687 p value
#marker_all_day_24_first_all
...
```
```{r}
marker_plot_day_48_first=markers_first_elmir_48[[1]]

marker_plot_plot_day_48_first=markers_first_elmir_48[[2]]
marker_all_day_48_first=markers_first_elmir_48[[3]]
marker_all_day_48_first_5=marker_all_day_48_first[names(marker_all_day_48_first)==5]
```

```{r}
diff_marker=marker_all_day_48_first[names(marker_all_day_48_first)=="5"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(diff_marker,file="Precursor_markers_48h.csv",row.names = F, col.names = F)
```
```{r}
two_marker=marker_all_day_48_first[names(marker_all_day_48_first)=="3"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(two_marker,file="2CLC_markers_48h.csv",row.names = F, col.names = F)
```

```{r}
pluri_marker=marker_all_day_48_first[names(marker_all_day_48_first)=="2"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(pluri_marker,file="Pluripotent_markers_48h.csv",row.names = F, col.names = F)
```
...
test=Test_Fisher_0_48(final_cluster_version_sub,marker_all_day_24_first_all,marker_all_day_48_first_5)
test[[2]][test[[1]]<0.05&test[[2]]>2]
```

```{r}

marker_all_day_48_first_3=marker_all_day_48_first[names(marker_all_day_48_first)==3]
test=Test_Fisher_0_48(final_cluster_version_sub,marker_all_day_24_first_all,marker_all_day_48_first_3)
test[[2]][test[[1]]<0.05&test[[2]]>2]
```

```{r}

marker_all_day_48_first_2=marker_all_day_48_first[names(marker_all_day_48_first)==2]
test=Test_Fisher_0_48(final_cluster_version_sub,marker_all_day_24_first_all,marker_all_day_48_first_2)
test[[2]][test[[1]]<0.05&test[[2]]>2]
```
...
listInput <- list(mouse_48h_differentiation=marker_all_day_48_first_5,mouse_24h_differentiation=marker_all_day_24_first_2, mouse_0h_differentiation=marker_all_day_0_first_2)

pdf("/Users/gabriele.lubatti/Downloads/common_0_24_48.pdf",width = 8, height = 6)
...
```{r }
final_cluster_version_sub[final_cluster_version_sub=="0"]="Pluripotent cells"
final_cluster_version_sub[final_cluster_version_sub=="1-step_2"]="2CLC"
final_cluster_version_sub[final_cluster_version_sub=="2-step_2"]="Precursor cells"


cluster_number_48_lett_new=cluster_number_48_lett[cluster_number_48_lett%in%c("C","D","F")]
cluster_number_48_lett_new[cluster_number_48_lett_new=="C"]="Pluripotent cells"
cluster_number_48_lett_new[cluster_number_48_lett_new=="D"]="2CLC"
cluster_number_48_lett_new[cluster_number_48_lett_new=="F"]="Precursor cells"

cluster_number_0_new=as.vector(as.factor(mayra_0_ciara$RNA_snn_res.0.1))
cluster_number_0_new[cluster_number_0_new==0]="Pluripotent cells"
cluster_number_0_new[cluster_number_0_new==1]="2CLC"
cluster_number_0_new[cluster_number_0_new==2]="Precursor cells"

time_0=rep("0h",length(cluster_number_0_new))
time_24=rep("24h",length(final_cluster_version_sub))
time_48=rep("48h",length(cluster_number_48_lett_new))

time_all=c(time_0,time_24,time_48)
composition=c(cluster_number_0_new,final_cluster_version_sub,cluster_number_48_lett_new)

composition=factor(composition,levels=c("Pluripotent cells","2CLC","Precursor cells"))
df_composition=data.frame(time_all,composition)

colnames(df_composition)=c("cluster","origin_space")

p=barplot_cell_cycle(df_composition[,2],df_composition[,1],"Mouse ESCs cell type composition",fraction = T)

pdf("/Users/gabriele.lubatti/Downloads/composition.pdf",width = 8, height = 6)
p[[1]]+theme(text = element_text(size=18),axis.text.x = element_text(size=18,angle = 45, vjust = 1, hjust = 1))+labs(fill="Composition")+ggtitle("Mouse ESCs treated with RA")+xlab(NULL)
dev.off()
...
p=plot_localized_genes_interactive(coordinate_umap,norm_elmir_ciara_5_30,rank_intersect,genes_name_text,hm.colors,min_x=NULL,max_x=NULL,min_y=NULL,max_y=NULL)
...
```{r}
yse1_marker=markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="YSE1"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(yse1_marker,file="YSE1_markers.csv",row.names = F, col.names = F)
```
...
pdf("/Users/gabriele.lubatti/Downloads/dpt_YSE1_sup.pdf",width = 8, height = 6)
...
```{r}
mep1_marker=markers_first_elmir_3_final[names(markers_first_elmir_3_final)=="MEP1"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(mep1_marker,file="MEP1_markers.csv",row.names = F, col.names = F)
```
...
plot_umap(umap_cordinate_heart_small,cluster_heart_small)
...
```{r}
two_marker=marker_heart[names(marker_heart)=="YSE1"]
#setwd("/Users/gabriele.lubatti/Downloads")
write.csv2(two_marker,file="YSE1_markers_mouse.csv",row.names = F, col.names = F)
```
...
name_mouse=read.csv("mega_convert_human.csv")
