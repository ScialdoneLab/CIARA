...
```{r }
load(system.file("extdata", "raw_counts_human_data.Rda", package ="CIARA"))
load(system.file("extdata", "hvg_human_gastrula.Rda", package ="CIARA"))
...
```{r }
  human_data_seurat <- cluster_analysis_integrate_rare(raw_counts_human_data, "Human_data", 0.1, 5, 30, feature_genes = hvg_human_gastrula)

human_data_seurat@assays$RNA@var.features <- hvg_human_gastrula
...
```{r }
norm_counts_small <- apply(norm_human_data[(ciara_genes),  ], 1, function(x) {
  y <- x/sum(x)
  return(y)
  })
...
plot_balloon_marker(norm_human_data[, grep(paste(toMatch, collapse="|"), final_cluster_human)], final_cluster_human_version_sub[grep(paste(toMatch, collapse="|"), final_cluster_human)], top_endo, 20, max_size=5, text_size=10)


toMatch <- c("Hemogenic Endothelial Progenitors")
plot_balloon_marker(norm_human_data[, grep(paste(toMatch, collapse = "|"), final_cluster_human)], final_cluster_human_version_sub[grep(paste(toMatch, collapse = "|"), final_cluster_human)], top_hemo, 20, max_size = 5, text_size = 8)


toMatch <- c("ExE Mesoderm")
plot_balloon_marker(norm_human_data[, grep(paste(toMatch, collapse = "|"), final_cluster_human)], final_cluster_human_version_sub[grep(paste(toMatch, collapse = "|"), final_cluster_human)], top_meso, length(top_meso), max_size = 5, text_size = 8)
